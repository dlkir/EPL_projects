...
{
  static constexpr bool value = (array_traits<T>::my_rank==1 
				||array_traits<T>::my_rank==2 
				||array_traits<T>::my_rank==3);
};

template<typename T1, typename T2>
struct is_both_numeric_type
{
  static constexpr bool both_numeric_value =is_numeric_type<T1>::value && is_numeric_type<T2>::value;
};

        template<typename T1, typename T2>
        struct binop_OK{
                static constexpr bool value = is_both_numeric_type<T1,T2>::both_numeric_value;
        };

        template<typename T1, typename T2, typename Op> struct result_type{};

        template<typename T1, typename T2, typename R2, typename Op>
        struct result_type<T1,valarray<T2,R2>,Op> {
                using Restype=valarray<typename choose_type<T1, T2>::type, Proxy<Op, T2, R2, T1>>;
        };

        template<typename T1, typename R1,typename T2, typename Op>
        struct result_type<valarray<T1,R1>,T2, Op> {
                using Restype=valarray<typename choose_type<T1, T2>::type, Proxy<Op, T1, R1, T2>>;
        };


        template<typename T1, typename R1, typename T2, typename R2, typename  Op>
        struct result_type<valarray<T1,R1>,valarray<T2,R2>, Op>{
                using Restype=valarray<typename choose_type<T1, T2>::type, Proxy<Op, T1, R1, T2, R2>>;
        };


        template <typename T1, typename T2, typename Op>
        using ResultType=typename result_type<T1,T2,Op>::Restype;
...
public:
	...
	valarray(OP op, const valarray<T1, R1>& x, const T2& y) : R(op, x, scalar<T2>(y), true) {}
        ...
        template <typename OP, typename T1, typename T2, typename R2>
        valarray(OP op, const T1& x, const valarray<T2, R2>& y) : R(op, y, scalar<T2>(x), false) {}
...
/*********************************************/
	...
	if(this->size()==0){
	    ...
	    for (int i=0; i < that.size()&& i<this->size(); ++i){
		(*this)[i] = (T)that[i];} //is type conversion OK?
	}
...
#define _DEFINE_BINARY_OPERATOR(_Op, _Name)                             \
  template<typename T1, typename T2>          \
    typename EnableIf<binop_OK<T1,T2>::value, ResultType<T1,T2, std::_Name<typename choose_type<T1, T2>::type> > >::type	\
    operator _Op(const T1& x, const T2& y)    \
    {                                                                   \
      return  ResultType<T1,T2, std::_Name<typename choose_type<T1, T2>::type> >(std::_Name<typename choose_type<T1, T2>::type>(),x,y);    \
    }                                                                   
...
/*
template<typename T1,typename T2>
typename EnableIf<BinopOK<T1,T2>(),ResultType<T1, T2,std::plus<void> > >::type
operator+(const T1& x, const T2& y) {
    return ResultType<T1,T2,std::plus<void>>(std::plus<void>(),x,y);
}

template<typename T1,typename T2>
typename EnableIf<BinopOK<T1,T2>(),ResultType<T1, T2, std::minus<void>> >::type
operator-(const T1& x, const T2& y) {
    return ResultType<T1,T2,std::minus<void>>(std::minus<void>(),x,y);
}

template<typename T1,typename T2>
typename EnableIf<BinopOK<T1,T2>(),ResultType<T1, T2, std::multiplies<void> > >::type
operator*(const T1& x, const T2& y) {
    return ResultType<T1,T2,std::multiplies<void>>(std::multiplies<void>(),x,y);
}

template<typename T1,typename T2>
typename EnableIf<BinopOK<T1,T2>(),ResultType<T1, T2, std::divides<void>>>::type
operator/(const T1& x, const T2& y) {
    return ResultType<T1,T2,std::divides<void>>(std::divides<void>(),x,y);
}
*/
...
/*
//operator +
template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::plus<RetType>,T1,T2> operator+(const valarray<T1>& x, const valarray<T2>& y) {
    return Proxy<std::plus<RetType>,T1,T2>(std::plus<RetType>(), x, y);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::plus<RetType>,T1,T2> operator+(const valarray<T1>& x, const T2& y) {
    return Proxy<std::plus<RetType>,T2,T1,true>(std::plus<RetType>(), scalar<T2>(y),x, false);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::plus<RetType>,T1,T2> operator+(const T1& x, const valarray<T2>& y) {
    return Proxy<std::plus<RetType>,T1,T2,true>(std::plus<RetType>(), scalar<T1>(x), y, true);
}


//operator -
template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::minus<RetType>,T1,T2> operator-(const valarray<T1>& x, const valarray<T2>& y) {
    return Proxy<std::minus<RetType>,T1,T2>(std::minus<RetType>(), x, y);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::minus<RetType>,T1,T2> operator-(const valarray<T1>& x, const T2& y) {
    return Proxy<std::minus<RetType>,T2,T1,true>(std::minus<RetType>(), scalar<T2>(y),x, false);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::minus<RetType>,T1,T2> operator-(const T1& x, const valarray<T2>& y) {
    return Proxy<std::minus<RetType>,T1,T2,true>(std::minus<RetType>(), scalar<T1>(x), y, true);
}


//operator *
template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::multiplies<RetType>,T1,T2> operator*(const valarray<T1>& x, const valarray<T2>& y) {
    return Proxy<std::multiplies<RetType>,T1,T2>(std::multiplies<RetType>(), x, y);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::multiplies<RetType>,T1,T2> operator*(const valarray<T1>& x, const T2& y) {
    return Proxy<std::multiplies<RetType>,T2,T1,true>(std::multiplies<RetType>(), scalar<T2>(y),x, false);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::multiplies<RetType>,T1,T2> operator*(const T1& x, const valarray<T2>& y) {
    return Proxy<std::multiplies<RetType>,T1,T2,true>(std::multiplies<RetType>(), scalar<T1>(x), y, true);
}
...
//operator /
template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::divides<RetType>,T1,T2> operator/(const valarray<T1>& x, const valarray<T2>& y) {
    return Proxy<std::divides<RetType>,T1,T2>(std::divides<RetType>(), x, y);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::divides<RetType>,T1,T2> operator/(const valarray<T1>& x, const T2& y) {
    return Proxy<std::divides<RetType>,T2,T1,true>(std::divides<RetType>(), scalar<T2>(y),x, false);
}

template <typename T1, typename T2, typename RetType=typename choose_type<T1, T2>::type>
Proxy<std::divides<RetType>,T1,T2> operator/(const T1& x, const valarray<T2>& y) {
    return Proxy<std::divides<RetType>,T1,T2,true>(std::divides<RetType>(), scalar<T1>(x), y, true);
}
*/
