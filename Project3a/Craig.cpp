...
{std::cout<<"Craig::encounter(ObjInfo&)"<<endl;
    if (info.species == species_name()) {
        ...
        return LIFEFORM_EAT;
        std::cout<<"Decide to Eat "<<info.species<<std::endl;
    }
}

void Craig::initialize(void) {
    std::cout<<"Craig::initialize"<<endl;
 ...
 */
Craig::Craig() {
    std::cout<<"Craig::Craig()"<<endl;
    SmartPointer<Craig> self = SmartPointer<Craig>(this);
    new Event(0, [self](void) { self->startup(); });
}

Craig::~Craig() {std::cout<<"Craig::~Craig()"<<endl;}

void Craig::startup(void) {
    std::cout<<"Craig::startup()"<<endl;
...
Color Craig::my_color(void) const {
    std::cout<<"Craig::my_color()"<<endl;
    return RED;
}

SmartPointer<LifeForm> Craig::create(void) {
    std::cout<<"Craig::create()"<<endl;
    return new Craig;
}


void Craig::hunt(void) {
    std::cout<<"Craig::hunt()"<<endl;
