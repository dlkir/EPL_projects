...
public:
  ...
  bool remove(const Point& pos, Obj& oldobj, std::function<void(void)>& invoke_this) {
    if (!in_bounds(pos)) return false;
    std::cout<<"Remove From: x["<<left()<<","<<right()<<"],y["<<bottom()<<","<<top()<<"]"<<std::endl;//for debug
...
void QuadTree<Obj>::update_position(const Point& pos_old, 
  ...
  assert(pos_old == leaf->obj_pos);
    ...
    std::cout << "Leaf Position: (" << leaf->obj_pos.xpos << ", " << leaf->obj_pos.ypos << ")" << std::endl;//for debug
    std::cout << "Parent Grid: x[" << parent->left()<< ","<<parent->right()<<"] y["<< parent->bottom()<<","<<parent->top()<<"]"<< std::endl;//for debug
