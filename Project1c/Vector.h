...
public:
	...
	void push_front(T&& x){
		if (this->size()==capacity){
			...
			new (storage+capacity-1-lenH) T((T&&)x);
		}
		lenH=lenH+1;
	}
	///////////////////////////////////////////////////
...
public:
    ...
    iterator(vector<T>* p_vector, uint64_t pos)
    : _pos(pos)
    , _p_vector (p_vector){}
    ...
    uint64_t get_pos() const{return _pos;}
    vector<T>* get_vec() const{return _p_vector;}
...
//the 6 done~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    iterator operator + (const uint64_t num) {
        iterator tmp;
        tmp._p_vector=this->_p_vector;
        tmp._pos = this->_pos - num;
        return tmp;
    }
...
public:
    ...
    const_iterator(const vector<T>* p_vector, uint64_t pos)
    : _pos(pos)
    , _p_vector (p_vector)
    ...
    {
        _pos = other.get_pos();
        _p_vector=other.get_vec();
        printf("copy construct const_iter from iter");
    }
    ...
    const_iterator operator + (const uint64_t num) {
        const_iterator tmp;
        tmp._p_vector=this->_p_vector;
        tmp._pos = this->_pos + num;
        return tmp;
