...
/* the IntExp is a simple wrapper that promotes an integer literal (or constant)
...
Var _X{}, _Y{}, _Z{};
    ...
    //    struct ConstantWrapper{//for the use of const int
    //        double val;
    //        ConstantWrapper(int v):val(v){}//promote 42 into this ConstantWrapper
    //        double operator()(double){return val;}
    //        using result_type=double;
    //    }
    //    template<typename T>
    //    Addfun<T, ConstantWrapper> operator+(int val, T f){
    //        return AddFun<T,ConstantWrapper>{f,ConstantWrapper{val}};
    //    }
    ...
    struct program{
        T _val;
        program<Tail...> _tail;
        program(T val,Tail... tail):_val(val), _tail(tail...){}
        void operator()(void){
            _val();
            _tail();
        }
